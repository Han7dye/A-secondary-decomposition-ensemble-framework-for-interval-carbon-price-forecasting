...
while ~stop_BEMD(r,MODE_COMPLEX,ndirs) && (k < MAXMODES+1 || MAXMODES == 0) && ~any(mask)
  ...
  if (max(abs(m))) < (1e-10)*(max(abs(x)))
    if ~stop_sift
      warning('BEMD:warning','forced stop of BEMD : too small amplitude')
    else
      disp('forced stop of BEMD : too small amplitude')
    end
    break
  end
  ...
  while ~stop_sift && nbit<MAXITERATIONS
    ...
    if display_sifting && ~MODE_COMPLEX
      ...
      if FIXE || FIXE_H
        display_BEMD_fixe(t,m,mp,r,envminp,envmaxp,envmoyp,nbit,k,display_sifting)
      else
        sxp=2*(abs(envmoyp))./(abs(envmaxp-envminp));
        sp = mean(sxp);
        display_BEMD(t,m,mp,r,envminp,envmaxp,envmoyp,s,sp,sxp,sdt,sd2t,nbit,k,display_sifting,stop_sift)
      end
    end
    ...
    if(nbit==(MAXITERATIONS-1) && ~FIXE && nbit > 100)
      if exist('s','var')
        warning('BEMD:warning',['forced stop of sifting : too many iterations... mode ',int2str(k),'. stop parameter mean value : ',num2str(s)])
      else
        warning('BEMD:warning',['forced stop of sifting : too many iterations... mode ',int2str(k),'.'])
      end
    end
...
function stop = stop_BEMD(r,MODE_COMPLEX,ndirs)
if MODE_COMPLEX
  for k = 1:ndirs
    phi = (k-1)*pi/ndirs;
...
function display_BEMD(t,m,mp,r,envmin,envmax,envmoy,s,sb,sx,sdt,sd2t,nbit,k,display_sifting,stop_sift)
subplot(4,1,1)
plot(t,mp);hold on;
...
function display_BEMD_fixe(t,m,mp,r,envmin,envmax,envmoy,nbit,k,display_sifting)
subplot(3,1,1)
plot(t,mp);hold on;
plot(t,envmax,'--k');plot(t,envmin,'--k');plot(t,envmoy,'r');
title(['IMF ',int2str(k),';   iteration ',int2str(nbit),' before sifting']);
...
if any(mask)
  ...
  opts.mask = 0;
  imf1 = BEMD(x+mask,opts);
  imf2 = BEMD(x-mask,opts);
  if size(imf1,1) ~= size(imf2,1)
    warning('BEMD:warning',['the two sets of IMFs have different sizes: ',int2str(size(imf1,1)),' and ',int2str(size(imf2,1)),' IMFs.'])
  end
  S1 = size(imf1,1);
  S2 = size(imf2,1);
  if S1 ~= S2
